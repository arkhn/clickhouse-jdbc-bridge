#!/usr/bin/env bash
# Comprehensive 5-phase grid search:
#   Phase 1 — GC profile (6 + ZGC sanity), fixed Xmx=4g + default batch/fetch/max_block
#   Phase 2 — Xmx under winning GC, fixed defaults
#   Phase 3 — batch_size × fetch_size × max_block_size under winning GC+Xmx
#   Phase 4 — concurrency sweep at winning JVM + params
#   Phase 5 — workload-type sensitivity (point / small / full scan) at winning everything
#
# Each cell runs `clickhouse benchmark --timelimit 30s` against mssql.
# CSV: phase,gc,xmx,batch,fetch,max_block,concurrency,workload,queries,qps,p99_s,heap_max_b,gc_pause_per_s
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
COMPOSE="$SCRIPT_DIR/docker-compose.yml"
OVERRIDE="$SCRIPT_DIR/docker-compose.grid.yml"
LABEL="${1:-grid-v2-$(date -u +%Y%m%dT%H%M%SZ)}"
